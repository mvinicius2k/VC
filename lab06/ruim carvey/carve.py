#arquivo de https://pypi.org/project/seam-carving/ modificado para 
#retornar como máscara  o caminho achado pelo seam carving (somente largura)
...
def _resize_width(src: np.ndarray, width: int, energy_mode: str,
    ...
    src_w = src.shape[1]
    if src_w < width:
        dst, seams = _expand_width(src, width - src_w, energy_mode, keep_mask)
    else:
        dst, seams = _reduce_width(src, src_w - width, energy_mode, keep_mask)
    return dst, seams
...
def _resize_height(src: np.ndarray, height: int, energy_mode: str,
    ...
    if src.ndim == 3:
        src, seams = _resize_width(src.transpose((1, 0, 2)), height, energy_mode,
                            keep_mask)
        src.transpose((1, 0, 2))
        seams.transpose()
        
    else:
        src, seams = _resize_width(src.T, height, energy_mode, keep_mask).T
    return src, seams
...
def resize(src: np.ndarray, size: Tuple[int, int],
    ...
    if order == WIDTH_FIRST:
        if src.shape[1] != width:
            src, seam = _resize_width(src, width, energy_mode, keep_mask)
        if src.shape[0] != height:
            src, seam = _resize_height(src, height, energy_mode, keep_mask)
    else:
        if src.shape[0] != height:
            src, seam = _resize_height(src, height, energy_mode, keep_mask)
        if src.shape[1] != width:          
